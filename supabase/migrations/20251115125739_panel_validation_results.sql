/*
  ...
  1. New Tables
    - `panel_validation_results`
      ...
      - `job_id` (uuid) - Reference to job that generated this panel
  ...
  2. Security
    ...
    - Add policy for service role to insert and read validation results
...
CREATE TABLE IF NOT EXISTS panel_validation_results (
  ...
  facial_consistency integer NOT NULL CHECK (facial_consistency >= -1 AND facial_consistency <= 100),
  body_proportion_consistency integer NOT NULL CHECK (body_proportion_consistency >= -1 AND body_proportion_consistency <= 100),
  clothing_consistency integer NOT NULL CHECK (clothing_consistency >= -1 AND clothing_consistency <= 100),
  color_palette_consistency integer NOT NULL CHECK (color_palette_consistency >= -1 AND color_palette_consistency <= 100),
  art_style_consistency integer NOT NULL CHECK (art_style_consistency >= -1 AND art_style_consistency <= 100),
