...
export class ComicGenerationEngine {
  ...
  async generateScenesWithAudience(options: SceneGenerationOptions): Promise<SceneGenerationResult> {
    try {
      ...
      const characterDNA = options.enhancedContext?.characterDNA;
      if (!characterDNA) {
        throw new Error(
          'CRITICAL: Character DNA not provided in enhancedContext. Character DNA from PHASE 3 is required for quality consistency. Job must fail.'
        );
      }
